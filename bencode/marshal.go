...
//将文本转成struct或slice，slice可能为空，因此自己生成slice再赋值
func Unmarshal(r io.Reader, s interface{}) error {
	...
	if p.Kind() != reflect.Ptr {
		return errors.New("does must be a pointer")
	}
	switch o.type_ {
	case BLIST:
		list, _ := o.List()
...
func UnmarshalList(p reflect.Value, list []*BObject) error {
	if p.Kind() != reflect.Ptr || p.Elem().Type().Kind() != reflect.Slice {
		return errors.New("does must be a pointer to slice")
	}
	v := p.Elem()
	if len(list) == 0 {
		return nil
	...
	case BLIST:
		for i, o := range list {
			...
			ls := reflect.MakeSlice(v.Type().Elem(), len(val), len(val)) //slice可能为空
			lp.Elem().Set(ls)
			err = UnmarshalList(lp, val) //递归调用
			if err != nil {
				return err
			}
...
func UnmarshalDict(p reflect.Value, dict map[string]*BObject) error {
	if p.Kind() != reflect.Ptr || p.Elem().Type().Kind() != reflect.Struct {
		return errors.New("does must be a pointer to struct")
	}
	v := p.Elem()
	for i, n := 0, v.NumField(); i < n; i++ {
		...
		key := ft.Tag.Get("bencode")
		if key == "" {
			key = strings.ToLower(ft.Name) //小写
		}
		fo := dict[key]
		if fo == nil {
			continue
